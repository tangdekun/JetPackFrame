package com.djt.base.utils.compression
...
class Luban private constructor(private val mCacheDir: File) {
    ...
    fun launch(): Luban {
        ...
        if (gear == Luban.FIRST_GEAR)
            Observable.just(mFile)
                .map(object : Function<String?, File> {
                    @Throws(Exception::class)
                    override fun apply(s: String): File? {
                        val file = File(s)
    ...
    fun asObservable(): Observable<File> {
        return if (gear == FIRST_GEAR)
            Observable.just(mFile).map(object : Function<String?, File> {
    ...
    fun asListObservable(): Observable<File> {
        return if (gear == FIRST_GEAR)
            Observable.fromIterable(mListFile).map(object : Function<String, File> {
                @Throws(Exception::class)
                override fun apply(s: String): File? {
    ...
    private fun thirdCompress(file: File): File? {
        val thumb = mCacheDir.getAbsolutePath() + File.separator +
        ...
        if (scale <= 1 && scale > 0.5625) {
            if (height < 1664) {
                ...
                size = width * height / Math.pow(1664.toDouble(), 2.toDouble()) * 150
                size = if (size < 60) 60.toDouble() else size
            } else if (height >= 1664 && height < 4990) {
                thumbW = width / 2
                thumbH = height / 2
                size = thumbW * thumbH / Math.pow(2495.toDouble(), 2.toDouble()) * 300
                size = if (size < 60) 60.toDouble() else size
            } else if (height >= 4990 && height < 10240) {
                thumbW = width / 4
                thumbH = height / 4
                size = thumbW * thumbH / Math.pow(2560.toDouble(), 2.toDouble()) * 300
                size = if (size < 100) 100.toDouble() else size
            } else {
            ...
            val multiple = Math.ceil(height / (1280.0 / scale)) as Int
            thumbW = width / multiple
            thumbH = height / multiple
    ...
    private fun firstCompress(file: File): File? {
        ...
        val filePath = file.getAbsolutePath()
        val thumbFilePath = mCacheDir.getAbsolutePath() + File.separator +
                (if (TextUtils.isEmpty(filename)) System.currentTimeMillis() else filename) + ".jpg"
    ...
    private fun compress(imagePath: String, width: Int, height: Int): Bitmap {
        ...
        val heightRatio = Math.ceil(options.outHeight.toDouble() / height.toDouble()) as Int
        val widthRatio = Math.ceil(options.outWidth.toDouble() / width.toDouble()) as Int
    ...
    companion object {
        ...
        private val FIRST_GEAR = 1
        val THIRD_GEAR = 3

        private val TAG = "smartcity"
        private val DEFAULT_DISK_CACHE_DIR = "smartcity_disk_cache"
        ...
        operator fun get(context: Context): Luban {
            if (INSTANCE == null) INSTANCE = Luban.getPhotoCacheDir(context)?.let { Luban(it) }
            return INSTANCE!!
        }
