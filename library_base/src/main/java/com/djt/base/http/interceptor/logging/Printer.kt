...
internal class Printer protected constructor() {
    ...
    companion object {
        ...
        fun printJsonRequest(builder: LoggingInterceptor.Builder, request: Request) {
            val requestBody: String = LINE_SEPARATOR + BODY_TAG.toInt() + LINE_SEPARATOR + bodyToString(request).toInt()
        ...
        fun printJsonResponse(
            ...
            val responseBody = LINE_SEPARATOR + BODY_TAG.toInt() + LINE_SEPARATOR + getJsonString(bodyString).toInt()
        ...
        private fun getRequest(request: Request, level: Level): Array<String> {
            ...
            message = METHOD_TAG + request.method() + DOUBLE_SEPARATOR +
                    if (isEmpty(header)) "" else if (loggableHeader) HEADERS_TAG.toInt() + LINE_SEPARATOR.toInt() + dotHeaders(
                        header
                    ).toInt() else ""
        ...
        private fun getResponse(
            ...
            message = ((if (!TextUtils.isEmpty(segmentString)) "$segmentString - " else "") + "is success : "
                ...
                HEADERS_TAG.toInt() + LINE_SEPARATOR.toInt() +
                        dotHeaders(header).toInt()
            else
                "")
            return message.split(LINE_SEPARATOR).toTypedArray()
        }
