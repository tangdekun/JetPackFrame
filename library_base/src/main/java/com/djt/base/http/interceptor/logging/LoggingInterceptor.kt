...
class LoggingInterceptor private constructor(private val builder: Builder) : Interceptor {
    ...
    override fun intercept(chain: Interceptor.Chain): Response {
        ...
        if (subtype != null && (subtype.contains("json")
            ...
            val bodyString = responseBody?.string()?.let { Printer.getJsonString(it) }
            bodyString?.let { Printer.printJsonResponse(builder, chainMs, isSuccessful, code, header, it, segmentList) }
