...
object CustomActivityOnCrash {
    ...
    private val activityLog: ArrayDeque<String> = ArrayDeque(MAX_ACTIVITIES_IN_LOG)
    ...
    fun install(context: Context?) {
        try {
            if (context == null) {
                ...
                if (oldHandler != null && oldHandler::class.java.name.startsWith(CAOC_HANDLER_PACKAGE_NAME)) {
                    Log.e(TAG, "CustomActivityOnCrash was already installed, doing nothing!")
                } else {
                    if (oldHandler != null && !oldHandler::class.java.name.startsWith(DEFAULT_HANDLER_PACKAGE_NAME)) {
                        Log.e(
                            TAG,
                    ...
                    application!!.registerActivityLifecycleCallbacks(object : Application.ActivityLifecycleCallbacks {
                        ...
                        override fun onActivityCreated(activity: Activity, savedInstanceState: Bundle) {
                            if (activity::class.java !== config.errorActivityClass) {
                                // Copied from ACRA:
                            ...
                            if (config.isTrackActivities) {
                                activityLog.add(dateFormat.format(Date()) + ": " + activity::class.java.name + " created\n")
                            }
                        }
                        ...
                        override fun onActivityResumed(activity: Activity) {
                            if (config.isTrackActivities) {
                                activityLog.add(dateFormat.format(Date()) + ": " + activity::class.java.name + " resumed\n")
                            }
                        }
                        ...
                        override fun onActivityPaused(activity: Activity) {
                            if (config.isTrackActivities) {
                                activityLog.add(dateFormat.format(Date()) + ": " + activity::class.java.name + " paused\n")
                            }
                        }
                        ...
                        override fun onActivityDestroyed(activity: Activity) {
                            if (config.isTrackActivities) {
                                activityLog.add(dateFormat.format(Date()) + ": " + activity::class.java.name + " destroyed\n")
                            }
                        }
                    })
                }
    ...
    fun getConfigFromIntent(intent: Intent): CaocConfig {
        return intent.getSerializableExtra(CustomActivityOnCrash.EXTRA_CONFIG) as CaocConfig
    }
    ...
    private fun getBuildDateAsString(context: Context, dateFormat: DateFormat): String? {
        var buildDate: Long
        try {
            val ai = context.getPackageManager().getApplicationInfo(context.getPackageName(), 0)
    ...
    private fun getRestartActivityClassWithIntentFilter(context: Context): Class<out Activity>? {
        val searchedIntent = Intent().setAction(INTENT_ACTION_RESTART_ACTIVITY).setPackage(context.getPackageName())
        val resolveInfos = context.getPackageManager().queryIntentActivities(
            searchedIntent,
    ...
    private fun getLauncherActivity( context: Context): Class<out Activity>? {
        ...
        if (intent != null) {
            try {
                return Class.forName(intent!!.getComponent().getClassName()) as Class<out Activity>
            } catch (e: ClassNotFoundException) {
                //Should not happen, print it to the log!
                Log.e(
                    TAG,
    ...
    private fun getErrorActivityClassWithIntentFilter(context: Context): Class<out Activity>? {
        val searchedIntent = Intent().setAction(INTENT_ACTION_ERROR_ACTIVITY).setPackage(context.getPackageName())
        val resolveInfos = context.getPackageManager().queryIntentActivities(
            searchedIntent,
            PackageManager.GET_RESOLVED_FILTER
        )
